...
void pseudo_merge_opt::init(){
    init_base();
}

void pseudo_merge_opt::reshape(int num, int channel, int height, int width){
    if (!reshape_base(num, channel, height, width)) return; 
    switch(context_version_){
        case 0:
            hindex_ = pctx_->produce_param_fill(num,height,width);
            break;
        case 1:
            hindex_ = pectx_->produce_param_fill(num,height,width);
            break;
        default:
...
void pseudo_merge_opt::reshape_top(at::TensorOptions option){
    std::vector<std::vector<int64_t>> shapes;
    shapes.push_back({nout_,channel_, h_out_, w_out_});
    reshape_top_base(option,shapes);
}

void pseudo_merge_opt::reshape_bottom(at::TensorOptions option){
    std::vector<std::vector<int64_t>> shapes;
...
__global__ void pseudo_merge_forward_kernel(const int nthreads,  const scalar_t * const input, scalar_t * const output,
...
template <typename scalar_t>
__global__ void pseudo_merge_backward_kernel(const int nthreads,  scalar_t * const input, const scalar_t * const output,
    const int * hindex, const int width, const int height, const int channel, const int npart, const int h_out) {
    CUDA_KERNEL_LOOP(index, nthreads) {
        int pw = index % width;
        int ph = (index / width) % height;
...
std::vector<at::Tensor>  pseudo_merge_opt::backward_cuda(at::Tensor  top_diff) 
{
    int count;
    reshape_bottom(top_diff.options());
	AT_DISPATCH_FLOATING_TYPES(
		top_diff.scalar_type(), "pseudo_merge_backward_cuda", 
			([&] {
                    count = num_ * channel_ * width_ * height_;
                    caffe_gpu_set(stream_, count, scalar_t(0), bottom_diff_[0].data_ptr<scalar_t>());
                    pseudo_merge_backward_kernel<< <CAFFE_GET_BLOCKS(count), CAFFE_CUDA_NUM_THREADS, 0, stream_ >> >
