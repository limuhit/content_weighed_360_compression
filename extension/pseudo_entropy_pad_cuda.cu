...
void pseudo_entropy_pad_opt::reshape(int num, int channel, int height, int width){
    ...
    std::vector<at::Tensor> tmp = ctx_->produce_param(num, channel_, height_, width_, pad_);
    //printf("here %d %d %d %d %d %d\n",num_,channel_,h_out_,w_out_,height_,width_);
    param_ = tmp[0];
    inv_param_ = tmp[1];
    hindex_  = tmp[2];
    hindex2_ = tmp[3];
...
__global__ void pseudo_entropy_pad_forward_kernel(const int nthreads, const scalar_t* const input,  
     scalar_t * const output, const double * param, const int * hindex, const int * hindex2, 
    ...
    CUDA_KERNEL_LOOP(index, nthreads) {
        ...
        //int tn = pn / npart;
        //int tg = pn % npart;
        if (pw>=hindex[pn]) continue;
        int base = (pn * inner_shape + ps)*4;
        int qg = hindex2[base/4/width];
        int pbase = static_cast<int>(param[base]+1e-6) + pc*astride;
        if(qg==-1){
            output[pbase + pw + pad] = 0;
            continue;
        }
        int qbase = static_cast<int>(param[base+1]+1e-6) + pc*bstride;
		...
		bottom_data.scalar_type(), "pseudo_entropy_pad_forward_cuda", 
			([&] {
                ...
                pseudo_entropy_pad_forward_kernel<< <CAFFE_GET_BLOCKS(count), CAFFE_CUDA_NUM_THREADS, 0, stream_ >> >
                    (count, bottom_data.data_ptr<scalar_t>(), top_data_[0].data_ptr<scalar_t>(),param_.data_ptr<double>(), 
...
__global__ void pseudo_entropy_pad_backward_kernel(const int nthreads, scalar_t* const input,  
    const scalar_t * const output, const int * hindex, const double * inv_param, const int * stride_inv, 
    ...
    CUDA_KERNEL_LOOP(index, nthreads) {
        ...
        int pc = (index / inner_shape) % channel;
        int pn = index / inner_shape / channel;
        int pg = ph / height + pn*npart;
        int ps = pg*channel+pc;
        int pph = ph%height;
        ...
        int base = (pn*height*npart+ph)*stride + pw*stride_inv[pg];
        int nn = static_cast<int>(inv_param[base]+1e-6);
        for(int i=0, sbase; i<nn; i++){
            sbase = static_cast<int>(inv_param[base+i*2+2]+1e-6) + pc*astride_inner;
		...
		top_diff.scalar_type(), "ppseudo_entropy_pad_backward_cuda", 
			([&] {
                ...
                pseudo_entropy_pad_backward_kernel<< <CAFFE_GET_BLOCKS(count), CAFFE_CUDA_NUM_THREADS, 0, stream_ >> >
                    ...
                    inv_param_.data_ptr<double>(), stride_inv_.data_ptr<int>(), stride, width_, height_, channel_, 
