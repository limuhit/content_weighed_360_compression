...
import sys
import torch
from SphereDataset import load_test_dataset_large
...
def test_epoch_detail(epoch, test_dataloader, model, criterion,pr_scr,pr_dst,log,logw,args):
    ...
    with torch.no_grad():
        for idx,d in enumerate(test_dataloader):
            ...
            #img_name = f'{args.img_dir}/{idx+1}_{int(out_criterion["bpp_loss"]*1000)}_ourp.png'
            #print(img_name)
...
def parse_args(argv):
    ...
    parser.add_argument('--viewport_size', type=int, default = 342, metavar='viewport', 
...
def Job(args):
    ...
    test_dataloader = load_test_dataset_large(args.test_batch_size)
