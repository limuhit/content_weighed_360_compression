...
class MultiProjectM(BaseOpModule):
    ...
    def __init__(self, h, w, thetas, phis, fov = 0.6, near=False, device_id =0, time_flag=False):
        ...
        self.op = {gid:PCONV2.ProjectsOp(int(h), int(w), thetas, phis, fov, near, gid, time_flag) for gid in self.device_list}
...
class MultiProject(BaseOpModule):
    ...
    def __init__(self, h, w, fov = 0.6, near=False, device_id =0, time_flag=False):
        ...
        self.op = {gid:PCONV2.ProjectsOp(int(h), int(w), self.thetas, self.phis, fov, near, gid, time_flag) for gid in self.device_list}
